...
def _(mo):
    ...
    2. **Standardise and combine the files into one master analysis dataframe**
    3. **Filter the combined data interactively**
    4. **Build a ratio using selected numerator and denominator metrics**
    5. **Create a chart from either the filtered data or the ratio output**
    ...
    This version has been simplified so that it only works with **uploaded CSV files**.
    """)
    return
...
def _(mo):
    ...
    - align the schemas across files
...
def _(mo):
    ...
    - **Cover type**: filters by insurance cover grouping if available in the source data
    - **Source file**: lets you isolate values from specific uploaded CSV files
    - **Metric category**: a broad grouping inferred from the metric name, such as premium, claims, expense, or exposure
    - **Metric name**: the individual metric label used for actual analysis and ratio building

    A common approach is to start broad, then narrow down by **metric category** and **metric name**.
    """)
    return
...
def _(
    ...
    def _():
        # Make sure dependency is recognised
        _ = master_analysis_df

        # If no data yet, just pass through
        if master_analysis_df is None or master_analysis_df.empty:
            filtered_df = master_analysis_df.copy()

        else:
            # ✅ Safe handling of year_range (prevents NameError)
            if "year_range" in globals():
                if hasattr(year_range, "value") and isinstance(year_range.value, tuple):
                    year_range_value = year_range.value
                else:
            ...
            filtered_df = apply_filters(
                ...
                period_types=period_type_filter.value if "period_type_filter" in globals() else [],
                cover_types=cover_type_filter.value if "cover_type_filter" in globals() else [],
                source_files=source_file_filter.value if "source_file_filter" in globals() else [],
                metric_categories=metric_category_filter.value if "metric_category_filter" in globals() else [],
                metric_names=metric_name_filter.value if "metric_name_filter" in globals() else [],
            )
        return
...
def _(
    ...
    # Ensure dependency is recognised
    _ = master_analysis_df
    ...
    if master_analysis_df is None or master_analysis_df.empty:
        ...
        # ✅ Safe year_range handling
        if "year_range" in globals() and hasattr(year_range, "value") and isinstance(year_range.value, tuple):
            year_range_value = year_range.value
        else:
            year_range_value = None

        # ✅ Safe filters (only use if they exist)
        period_types = period_type_filter.value if "period_type_filter" in globals() else []
        cover_types = cover_type_filter.value if "cover_type_filter" in globals() else []
        source_files = source_file_filter.value if "source_file_filter" in globals() else []
        metric_categories = metric_category_filter.value if "metric_category_filter" in globals() else []
...
def _(mo):
    ...
    **sum(selected numerator values) / sum(selected denominator values)**
    ...
    This simplified version removes manual aggregation and grouping complexity to make the ratio builder easier to use.
    """)
    return
...
def _(filtered_df, mo, numeric_sorted_unique, safe_sorted_unique):
    if filtered_df is None or filtered_df.empty:
        ...
        calculation_view_options = ["Overall"]
        for option in ["year_clean", "quarter", "half_year", "period_type", "covertype", "source_file", "metric_source_table"]:
...
def _(mo):
    ...
    - **Filtered data**: the row-level dataset after applying the filters
