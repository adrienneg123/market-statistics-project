...
def _(pd, re):
    ...
    def build_master_analysis_df(raw_df, np, pd, keep_debug_cols=False):
        ...
        By default, this returns a slim analysis-ready dataframe.


        Set keep_debug_cols=True if you want to retain source/debug columns.
        ...
        # Drop rows that have neither a metric nor a value
...
def _(master_analysis_df, master_raw_df, mo, pd):
    ...
    if master_analysis_df is not None and not master_analysis_df.empty:


        cols_to_show = [c for c in preview_cols if c in master_analysis_df.columns]


        preview_df = master_analysis_df[cols_to_show].head(50)
        ...
        preview_df = pd.DataFrame({"info": ["No data loaded"]})


        preview_message = "No master dataframe available yet."
...
def _(filtered_df, mo, pd):
    ...
    if filtered_df is not None and not filtered_df.empty:
        ...
        filtered_cols_to_show = [
            ...
            c for c in filtered_preview_cols if c in filtered_df.columns
...
def _(filtered_df, mo):
    ...
    if filtered_df is None or filtered_df.empty:
        ...
        available_dimensions = [
            ...
            c for c in preferred_dimensions if c in filtered_df.columns


        ]


        value_options = ["value"] if "value" in filtered_df.columns else []
    ...
    row_dimensions = mo.ui.multiselect(
        ...
        value=[c for c in ["year_clean", "metric_name"] if c in available_dimensions],
    ...
    column_dimensions = mo.ui.multiselect(
        ...
        value=[c for c in ["quarter"] if c in available_dimensions],
    ...
    value_fields = mo.ui.multiselect(
        ...
        value=["value"] if "value" in value_options else [],
...
def _(filtered_df, mo, pd, summary_table_df):
    ...
    if filtered_df is not None and not filtered_df.empty:
        ...
        summary_cols_to_show = [


            c for c in summary_preview_cols if c in filtered_df.columns


        ]


        summary_filtered_preview = filtered_df[summary_cols_to_show].head(100)


    else:


        summary_filtered_preview = pd.DataFrame(


            {"info": ["No filtered data available"]}
    ...
    mo.vstack(
        ...
        [
            ...
            else pd.DataFrame({"info": ["No summary table output"]}),


            mo.md("### Underlying filtered data used for the table"),


            summary_filtered_preview,
...
def _(
    ...
    if (
        ...
        if chart_df is None or chart_df.empty:
            ...
            # ✅ ADD selection (this fixes your issue)
            ...
            # ✅ chart type + visibility for selection
